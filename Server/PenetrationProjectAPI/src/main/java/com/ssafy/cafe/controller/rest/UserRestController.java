...
public class UserRestController {
	...
	@GetMapping("/stamp/{userId}")
	@ApiOperation(value = "{userId}에 해당하는 Stamp개수를 반환한다.", response = List.class)
	public int getStampByUser(@PathVariable String userId) {
		List<Stamp> list = sService.selectByUser(userId);
		int stamp = 0;
		for(Stamp s : list) {
			stamp += s.getQuantity();
		}
